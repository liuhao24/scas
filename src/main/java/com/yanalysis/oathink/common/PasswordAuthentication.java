...
//		byte[] dk = pbkdf2(password, salt, 1 << cost);
		...
		return MD5Util.encodeByMD5(hash);
...
//		return ID + cost + '$' + enc.encodeToString(hash);
	 ...
	 */
	public boolean authenticate(byte[] password, String token, byte[] salt) {
		byte[] hash = new byte[password.length + salt.length];
		System.arraycopy(salt, 0, hash, 0, salt.length);
		System.arraycopy(password, 0, hash, salt.length, password.length);
		return MD5Util.encodeByMD5(hash).equals(token);
...
//		return zero == 0;
	...
	public static void main(String[] args) {
		...
		byte[] salt = "pSx1fqyv5".getBytes();
		byte[] hash = new byte[password.length + salt.length];
		System.arraycopy(salt, 0, hash, 0, salt.length);
		System.arraycopy(password, 0, hash, salt.length, password.length);
		System.out.println(MD5Util.encodeByMD5(hash));
	}
}
